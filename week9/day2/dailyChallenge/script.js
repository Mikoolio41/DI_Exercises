...
// // output
const makeAllCaps = (arrayWords) => {
  return new Promise((resolve, reject) => {
    if (arrayWords.every((word) => typeof word == "string")) {
      resolve(arrayWords.map((item) => item.toUpperCase()));
    }
...
let arrayUppercase = [];
const sortWords = (arrayUppercase) => {
  return new Promise((resolve, reject) => {
    if (arrayUppercase.length > 4) {
      resolve(arrayUppercase.sort());
    }
  ...
  return new Promise((resolve, reject) => {
    ...
    newArray.forEach((item) => {
      if (morseJS.hasOwnProperty(item)) {
        translate.push(morseJS[item]);
      } else {
        reject(`the ${item} is not in the morse code`);
      }
    });
    resolve(translate);
  });
